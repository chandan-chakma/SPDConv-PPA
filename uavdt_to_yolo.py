...
def parse_gt_file(gt_file):
    """Parse UAVDT ground truth file Returns: dict with frame_id as key and list of annotations as value.
    """
    annotations = {}
    ...
    with open(gt_file) as f:
        for line in f:
            ...
            annotations[frame_id].append({"bbox": bbox, "category": category, "occlusion": occlusion})
        ...
        if frame_id in annotations:
            with open(output_label_path, "w") as f:
                for ann in annotations[frame_id]:
                    ...
                    x_center, y_center, w, h = convert_bbox_to_yolo(img_width, img_height, ann["bbox"])
...
def main():
    ...
    # Get all sequences
    sequences = sorted([d for d in os.listdir(benchmark_dir) if os.path.isdir(os.path.join(benchmark_dir, d))])

...
def test_multiple_scales(weights, data='VisDrone.yaml'):
    ...
    for imgsz in scales:
        ...
        if results:
            results_dict[imgsz] = {
                'mAP@0.5': results.box.map50 * 100,
                'mAP@0.5-0.95': results.box.map * 100
            }
    ...
    for imgsz, metrics in results_dict.items():
        print(f"{imgsz}px: mAP@0.5={metrics['mAP@0.5']:.2f}%, "
...
if __name__ == '__main__':
    ...
    parser.add_argument('--multi-scale', action='store_true',
                       help='Test at multiple scales (640, 800, 1024)')
    
    # Visualization
    parser.add_argument('--visualize', type=str, default=None,
                       help='Path to images/video for visualization')
    ...
    if args.compare:
        compare_models(
            rsp_weights=args.weights,
            baseline_weights=args.compare,
            data=args.data
        )

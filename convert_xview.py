...
def convert_labels(fname=Path("datasets/xView/xView_train.geojson")):
    ...
    xView has 60 classes (originally numbered 11-94 in the dataset).
    This function maps them to 0-59 for YOLO.
    """
    
    path = fname.parent
    
    print(f"\n{'='*70}")
    print(f"Converting xView Dataset to YOLO Format")
    print(f"{'='*70}\n")
    ...
    if labels.exists():
        print(f"🗑️  Removing existing labels directory...")
    ...
    xview_class2index = [
        ...
        0, 1, 2, -1, 3, -1, 4, 5, 6, 7, 8, -1, 9, 10, 11,  # 11-25
        12, 13, 14, 15, -1, -1, 16, 17, 18, 19, 20, 21, 22, -1,  # 26-39
        23, 24, 25, -1, 26, 27, -1, 28, -1, 29, 30, 31, 32, 33,  # 40-53
        34, 35, 36, 37, -1, 38, 39, 40, 41, 42, 43, 44, 45, -1,  # 54-67
        -1, -1, -1, 46, 47, 48, 49, -1, 50, 51, -1, 52, -1, -1,  # 68-81
        -1, 53, 54, -1, 55, -1, -1, 56, -1, 57, -1, 58, 59  # 82-94
    ]
...
def reorganize_images(base_path):
    """
    Reorganize images into YOLO-compatible structure:
    datasets/xView/
    ├── images/
    │   ├── train/
    │   └── val/
    ...
    if train_src.exists() and not train_dst.exists():
        ...
        print(f"✅ Training images already in place")
    else:
    ...
    if val_src.exists() and not val_dst.exists():
        ...
        print(f"✅ Validation images already in place")
    else:
        print(f"⚠️  Validation images not found at {val_src}")
    
    print(f"\n{'='*70}")
    print(f"Image Reorganization Complete!")
    print(f"{'='*70}\n")


def create_autosplit_files(base_path, train_ratio=0.9):
    """
    Create train/val split files for YOLO.
    Since xView val set has no labels, we split the train set.
    """
...
def verify_conversion(base_path):
    ...
    if train_split.exists():
        ...
        print(f"❌ Train split file missing!")
    ...
    if val_split.exists():
        ...
        print(f"❌ Val split file missing!")
...
def main():
    ...
    if not xview_path.exists():
        ...
        print(f"Please download the xView dataset manually from:")
        print(f"https://challenge.xviewdataset.org")
        return
    ...
    print(f"python train.py --data xView.yaml")
    print("="*70 + "\n")

...
__all__ = ["MKSOM", "SOFP", "Bottleneck_RSP", "C2f_RSP", "FocalCIoULoss", "PixelDetHead"]
...
class MKSOM(nn.Module):
    ...
    Problem: Standard 3×3 convs destroy features of 6-8 pixel objects Solution: 4 parallel paths with different kernels
    (1×1, 3×3, 5×5, 7×7)
...
class SOFP(nn.Module):
    ...
    Problem: Standard FPN deeply processes features → destroys tiny objects Solution: Use SHALLOW features directly from
...
class PixelDetHead(nn.Module):
    ...
    Problem: YOLOv8 head uses 1×1 conv (no spatial context) + 16-bin DFL Solution: Use 3×3 conv (spatial context) +
    32-bin DFL (fine-grained)
    ...
    def __init__(self, nc=80, ch=(128, 128, 128)):  # P2, P3, P4 channels
        ...
        for c in ch:
            ...
            self.cls_convs.append(nn.Sequential(Conv(c, c, 3, 1), nn.Conv2d(c, self.nc, 1)))

            # Regression: 32-bin DFL for fine-grained localization
            self.reg_convs.append(nn.Sequential(Conv(c, c, 3, 1), nn.Conv2d(c, 4 * self.reg_max, 1)))
...
class FocalCIoULoss(nn.Module):
    ...
    Problem: Standard CIoU treats all objects equally → large objects dominate Solution: Weight loss by object size →
    small objects get higher weight
    ...
    def compute_ciou(self, box1, box2):
        ...
        v = (4 / (3.14159**2)) * torch.pow(torch.atan(w2 / (h2 + self.eps)) - torch.atan(w1 / (h1 + self.eps)), 2)
...
class C2f_RSP(nn.Module):
    ...
    This is the main building block used throughout RSP-YOLO backbone Replaces all C2f modules in standard YOLOv8
    """
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(Bottleneck_RSP(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))

...
class AMKP(nn.Module):
    ...
    def __init__(self, channels):
        ...
        self.pool_small = nn.Sequential(
            nn.MaxPool2d(3, stride=1, padding=1),
            Conv(channels, channels, 1, 1)
        )
        self.pool_medium = nn.Sequential(
            nn.MaxPool2d(5, stride=1, padding=2),
            Conv(channels, channels, 1, 1)
        )
        self.pool_large = nn.Sequential(
...
class ScaleSelector(nn.Module):
    ...
    def __init__(self, channels, num_scales=3):
        ...
        self.predictor = nn.Sequential(
            ...
            Conv(channels // 2, num_scales, 1, 1),
            nn.Softmax(dim=1)
        )
...
class SSFP(nn.Module):
    ...
    def forward(self, x):
        ...
        selected = (feat_small * scale_map[:, 0:1, :, :] +
...
class C2f_MSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(
            Bottleneck_MSP(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) 
            for _ in range(n)
        )
...
__all__ = ['MSP', 'C2f_MSP', 'Bottleneck_MSP', 'PAConv', 'AMKP', 'SSFP']
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        return x * self.sigmoid(
            self.conv(torch.cat([torch.mean(x, dim=1, keepdim=True), torch.max(x, dim=1, keepdim=True)[0]], dim=1))
        )


__all__ = ["C2f_SP", "ChannelAttention", "SPBottleneck", "SpatialAttention"]

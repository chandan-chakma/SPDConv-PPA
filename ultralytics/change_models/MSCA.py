...
class PixelContextFeatureAggregator(nn.Module):
    """PCFA: Pixel-Context Feature Aggregator Preserves pixel-level information while capturing surrounding context
    ...
    def __init__(self, channels, reduction=4):
        ...
        self.pixel_conv = nn.Sequential(
            nn.Conv2d(channels, channels, 1), nn.BatchNorm2d(channels), nn.SiLU(inplace=True)
        )
        ...
        # Final fusion
        self.fusion = nn.Sequential(nn.Conv2d(channels * 4, channels, 1), nn.BatchNorm2d(channels))
...
class AdaptiveDetailAwareFusion(nn.Module):
    """ADAF: Adaptive Detail-Aware Fusion Dynamically adjusts feature fusion based on object scale Gives more weight to
    ...
    def __init__(self, channels):
        ...
        self.fusion = nn.Sequential(
            nn.Conv2d(channels * 2, channels, 1), nn.BatchNorm2d(channels), nn.SiLU(inplace=True)
        )
...
class HierarchicalScaleFocusedPyramid(nn.Module):
    """HSFPN: Hierarchical Scale-Focused Pyramid Network Enhanced FPN that focuses on small object scales Creates dense
    feature pyramid with emphasis on high-resolution levels.
    """
    ...
    def __init__(self, channels_list: list[int], out_channels: int = 256):
        ...
        self.lateral_convs = nn.ModuleList(
            [nn.Sequential(nn.Conv2d(ch, out_channels, 1), nn.BatchNorm2d(out_channels)) for ch in channels_list]
        )

        # Top-down pathway with ADAF fusion
        self.adaf_modules = nn.ModuleList([AdaptiveDetailAwareFusion(out_channels) for _ in range(self.num_levels - 1)])
    ...
    def forward(self, features: list[torch.Tensor]) -> list[torch.Tensor]:
        ...
        for i in range(self.num_levels - 1):
            ...
            if down.shape[2:] != top_down_features[i + 1].shape[2:]:
                down = F.interpolate(
                    down, size=top_down_features[i + 1].shape[2:], mode="bilinear", align_corners=False
                )
            fused = down + top_down_features[i + 1]
            bottom_up_features.append(fused)

        # Output refinement
        outputs = [output_conv(feat) for output_conv, feat in zip(self.output_convs, bottom_up_features)]

        return outputs


class MSCABlock(nn.Module):
    """Complete MSCA Block combining PCFA and ADAF Can be inserted into YOLOv8 backbone or neck.
    """
...
class SmallObjectDetectionHead(nn.Module):
    """Specialized detection head for small objects Reduces stride and increases resolution for tiny object detection.
    """
...
def test_modules():
    ...
    features = [torch.randn(2, 128, 80, 80), torch.randn(2, 256, 40, 40), torch.randn(2, 512, 20, 20)]

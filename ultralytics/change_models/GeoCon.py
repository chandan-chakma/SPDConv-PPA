...
class SemanticContextEncoder(nn.Module):
    """
    Encodes semantic geographic context from image features.
    Learns to predict: roads, water, buildings, vegetation, sky
    These are the key geographic contexts for overhead imagery.
    """
...
class GeographicPriorModule(nn.Module):
    """
    Uses geographic context to modulate features.
    Key insight: Small objects are more likely in certain geographic contexts:
    - Cars: on roads
    - Ships: in water
    - Planes: near airports/runways
    - People: on roads, buildings, not in water
    """
...
class ContextGuidedDetectionHead(nn.Module):
    """
    Detection head that uses geographic context for class-specific enhancement.
    Different object classes benefit from different geographic contexts.
    """
...
class GeoCon(nn.Module):
    """
    Main GeoCon module that integrates geographic context into feature extraction.
    This is the core novelty of GeoCon-YOLO.
    """
    ...
    def forward(self, x):
        ...
        x_enhanced, semantic_map = self.geo_prior(x)
...
class C2f_GeoCon(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(
            Bottleneck_GeoCon(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) 
            for _ in range(n)
        )
...
class GeoConLoss:
    """
    Additional loss for GeoCon training.
    Supervises semantic context learning.
    """
    ...
    def semantic_consistency_loss(self, semantic_maps, images):
        """
        Encourage semantic maps to be consistent and meaningful.
        Uses self-supervised learning with image statistics.
        """
...
def test_geocon():
    ...
    print("="*60)
    print(f"✓ SemanticContextEncoder: Learns geographic context")
    print(f"✓ GeographicPriorModule: Modulates features with context")
    print(f"✓ GeoCon: Main module for integration")
    print(f"✓ C2f_GeoCon: YOLOv8-compatible block")
    print(f"✓ GeoConLoss: Self-supervised semantic learning")
    print("="*60)

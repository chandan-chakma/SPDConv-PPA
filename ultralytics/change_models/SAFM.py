...
class SimpleSAFM(nn.Module):
    def __init__(self, dim, ratio=4):
        """初始化SimpleSAFM模块。 dim: 输入的特征图通道数。 ratio: 将通道数分为两部分的比例。.
        """
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x):
        """前向传播：进行特征提取与调制 x: 输入特征图，大小为 (batch_size, dim, H, W).
        """
...
class CCM(nn.Module):
    def __init__(self, dim, ffn_scale, use_se=False):
        """初始化CCM模块。 dim: 输入的特征图通道数。 ffn_scale: 用于计算隐藏层维度的比例。 use_se: 是否使用Squeeze-and-Excitation（SE）模块.
        """
        super().__init__()
        self.use_se = use_se
    ...
    def forward(self, x):
        """前向传播：进行特征混合 x: 输入特征图，大小为 (batch_size, dim, H, W).
        """
...
class AttBlock(nn.Module):
    ...
    def forward(self, x):
        """前向传播：依次经过SimpleSAFM和CCM模块 x: 输入特征图，大小为 (batch_size, dim, H, W).
        """
...
class SAFMNPP(nn.Module):
    def __init__(self, input_dim, dim, n_blocks=3, ffn_scale=1.5, use_se=False, upscaling_factor=2):
        """初始化SAFMNPP超分辨率模型。 input_dim: 输入图像的通道数。 dim: 网络中间层的通道数。 n_blocks: 注意力块的数量。 ffn_scale: 用于计算通道数的比例。 use_se:
        是否使用Squeeze-and-Excitation（SE）模块。 upscaling_factor: 超分辨率放大因子。.
        """
        super().__init__()
    ...
    def forward(self, x):
        """前向传播：输入图像经过多个模块进行超分辨率恢复 x: 输入图像，大小为 (batch_size, input_dim, H, W).
        """
        # 首先通过插值进行上采样

...
class ContextRangePredictor(nn.Module):
    """
    Context Range Predictor (CRP)
    Predicts optimal contextual range for each spatial location
...
class DensityEstimator(nn.Module):
    """
    Estimates local object density to modulate context range
    High density → reduce context (avoid confusion)
    Low density → expand context (gather more info)
    """
...
class AdaptiveDeformableConv(nn.Module):
    ...
    Dynamically adjusts sampling offsets based on context range and density
    """
    ...
    def _deform_conv(self, x, offset, modulation):
        """
        Simplified deformable convolution using grid_sample
        For production, use torchvision.ops.DeformConv2d
        """
        B, C, H, W = x.shape
...
class DAMCF(nn.Module):
    """
    Dynamic Adaptive Multi-Scale Contextual Fusion Module
    Main module that combines CRP + Density Estimation + Adaptive Deformable Conv
    """
    def __init__(self, in_channels, out_channels):
        ...
        # Adaptive Deformable Convolution
        self.adaptive_conv = AdaptiveDeformableConv(
            in_channels, 
            out_channels,
            kernel_size=3,
            stride=1,
...
class Bottleneck_DAMCF(nn.Module):
    """
    Bottleneck with DAMCF module
    Similar to Bottleneck_LWN pattern
    """
...
class C2f_DAMCF(nn.Module):
    """
    C2f module with DAMCF-enhanced bottlenecks
    Similar to C2f_LWN pattern
    """
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(
            Bottleneck_DAMCF(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) 
            for _ in range(n)
        )

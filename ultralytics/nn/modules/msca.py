...
class MSCABlock(nn.Module):
    """PCFA: Pixel-Context Feature Aggregator Preserves pixel-level information with multi-scale context.
    """
    ...
    def __init__(self, c1, c2):
        ...
        # Pixel pathway
        self.pixel_conv = nn.Sequential(nn.Conv2d(c, c, 1), nn.BatchNorm2d(c), nn.SiLU())

        # Local context (dilation=1)
        self.local_conv = nn.Sequential(
            nn.Conv2d(c, c, 3, padding=1, groups=c), nn.Conv2d(c, c, 1), nn.BatchNorm2d(c), nn.SiLU()
        )

        # Medium context (dilation=2)
        self.medium_conv = nn.Sequential(
            nn.Conv2d(c, c, 3, padding=2, dilation=2, groups=c), nn.Conv2d(c, c, 1), nn.BatchNorm2d(c), nn.SiLU()
        )

        # Wide context (dilation=3)
        self.wide_conv = nn.Sequential(
            nn.Conv2d(c, c, 3, padding=3, dilation=3, groups=c), nn.Conv2d(c, c, 1), nn.BatchNorm2d(c), nn.SiLU()
        )
        ...
        # Fusion
        self.fusion = nn.Sequential(nn.Conv2d(c * 4, c, 1), nn.BatchNorm2d(c))
...
class ADAFFusion(nn.Module):
    """ADAF: Adaptive Detail-Aware Fusion Dynamically fuses features based on scale.
    """
    ...
    def __init__(self, c1, c2):
        ...
        self.detail = nn.Sequential(
            nn.Conv2d(c2, c2, 3, padding=1), nn.BatchNorm2d(c2), nn.SiLU(), nn.Conv2d(c2, c2, 1), nn.BatchNorm2d(c2)
        )

        # Context enhancement (for low-res)
        self.context = nn.Sequential(
            nn.Conv2d(c2, c2, 3, padding=1, groups=c2), nn.Conv2d(c2, c2, 1), nn.BatchNorm2d(c2), nn.SiLU()
        )

        # Scale-aware gate
        self.gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1), nn.Conv2d(c2 * 2, c2, 1), nn.SiLU(), nn.Conv2d(c2, 2, 1), nn.Softmax(dim=1)
        )

        # Fusion
        self.fusion = nn.Sequential(nn.Conv2d(c2 * 2, c2, 1), nn.BatchNorm2d(c2), nn.SiLU())
...
class HSFPN(nn.Module):
    """HSFPN: Hierarchical Scale-Focused Pyramid Network Enhanced multi-scale feature pyramid with small object focus.
    """
    ...
    def __init__(self, c1, c2):
        ...
        # Lateral connections
        self.laterals = nn.ModuleList([nn.Conv2d(in_ch, out_channels, 1) for in_ch in in_channels_list])

        # Top-down fusion with ADAF
        self.fusions = nn.ModuleList([ADAFFusion(out_channels, out_channels) for _ in range(len(in_channels_list) - 1)])

        # Small object enhancement (PCFA on top 2 levels)
        self.enhance = nn.ModuleList(
            [MSCABlock(out_channels, out_channels) for _ in range(min(2, len(in_channels_list)))]
        )
        ...
        self.outputs = nn.ModuleList(
            [
                nn.Sequential(
                    nn.Conv2d(out_channels, out_channels, 3, padding=1), nn.BatchNorm2d(out_channels), nn.SiLU()
                )

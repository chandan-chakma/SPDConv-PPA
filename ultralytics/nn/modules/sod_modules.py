...
class WaveletDownsample(nn.Module):
    ...
    Unlike strided convolution which acts as a low-pass filter, wavelet transform decomposes signal into frequency
    bands:
    ...
    def dwt2d(self, x):
        """2D Discrete Wavelet Transform using Haar wavelet. Decomposes input into 4 frequency sub-bands.
        """
        _B, _C, H, W = x.shape
        ...
        ll = (x[:, :, 0::2, 0::2] + x[:, :, 1::2, 0::2] + x[:, :, 0::2, 1::2] + x[:, :, 1::2, 1::2]) / 4.0

        # LH - Horizontal details (horizontal edges)
        lh = (x[:, :, 0::2, 0::2] + x[:, :, 0::2, 1::2] - x[:, :, 1::2, 0::2] - x[:, :, 1::2, 1::2]) / 4.0

        # HL - Vertical details (vertical edges)
        hl = (x[:, :, 0::2, 0::2] + x[:, :, 1::2, 0::2] - x[:, :, 0::2, 1::2] - x[:, :, 1::2, 1::2]) / 4.0

        # HH - Diagonal details (diagonal edges - CRITICAL for small objects!)
        hh = (x[:, :, 0::2, 0::2] - x[:, :, 1::2, 0::2] - x[:, :, 0::2, 1::2] + x[:, :, 1::2, 1::2]) / 4.0
...
class AdaptiveReceptiveField(nn.Module):
    ...
    def __init__(self, channels, num_scales=4, reduction=16):
        ...
        # Feature fusion
        self.fusion = nn.Sequential(nn.Conv2d(channels, channels, 1), nn.BatchNorm2d(channels))
...
class SuperResolutionUpsample(nn.Module):
    ...
    Problem: Bilinear/Nearest upsampling loses high-frequency details Solution: Learn to reconstruct details using
    ...
    def __init__(self, in_channels, out_channels, scale_factor=2):
        ...
        self.skip_upsample = nn.Sequential(
            nn.Conv2d(in_channels, out_channels * (scale_factor**2), 1), nn.PixelShuffle(scale_factor)
        )
...
class ContextAugmentation(nn.Module):
    ...
    Problem: Small objects lack context Solution: Add global information using attention
    """
...
class C2f_SOD(nn.Module):
    """C2f module enhanced with Small Object Detection features. Combines: Adaptive RF + Context Augmentation.
    """
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(Bottleneck_SOD(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class C2f_Wavelet(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(Bottleneck_Wavelet(self.c, self.c, shortcut, g) for _ in range(n))
...
class WaveletDetailExtractor(nn.Module):
    ...
    def __init__(self, channels):
        ...
        self.detail_conv = nn.Sequential(
            nn.Conv2d(channels, channels, 3, padding=1, bias=False), nn.BatchNorm2d(channels), nn.SiLU(inplace=True)
        )
...
class EnhancedDetect(nn.Module):
    """Enhanced detection head with better small object handling. Features: - Decoupled classification and regression -
    Dynamic convolutions - Multi-scale fusion.
    """
...
class BiFPNBlock(nn.Module):
    """Bidirectional Feature Pyramid Network block. Better than PANet for multi-scale fusion.
    """
